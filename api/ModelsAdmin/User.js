export default (ORM = null, Role = null, ServiceCost= null) => {
  
  async function getServiceCosts() {
    let {data} = await ServiceCost.methods.all();
    let costs = data.data;
  ...
  async function getRoles() {
    let {data} = await Role.methods.all();
    let roles = data.data;
  ...
  return {
    ...
    form: async () => ({
      fields: [
        ...
        {
          label: 'Тип цен по умолчанию',
          type: 'select',
          placeholder: 'Тип цен по умолчанию',
          key: 'service_cost_id',
          items: await getServiceCosts(),
          multiple: false,
          disabled: false,
        },
        {
